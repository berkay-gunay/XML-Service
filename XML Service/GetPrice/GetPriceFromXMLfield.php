...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    //XML dosyanın yolunu güvenli şekilde alıyoruz
    $xmlFilePath = __DIR__ . '/data/Karnak_XML_Info_Test.xml'; // Gerçek yolu belirle
    $allowedBasePath = realpath(__DIR__ . '/data'); // Sadece bu klasör içindekilere izin ver

    $realPath = realpath($xmlFilePath);

    // Yol geçerli ve doğru dizin altında mı?
    if ($realPath && strpos($realPath, $allowedBasePath) === 0 && is_readable($realPath)) {
        $xml_data = simplexml_load_file($realPath);
    } else {
